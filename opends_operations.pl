...
# Potential other connections that might be useful.
#my $dsfilter = 'cn=Administration Connector 0.0.0.0 port 4444 Statistics';
my $search = $ldap->search(
    base   => 'cn=monitor',
    filter => $dsfilter,
    attrs  => [ "$h-abandon-$f", "$h-add-$f", "$h-bind-$f", "$h-compare-$f", "$h-delete-$f", "$h-extended-$f", "$h-mod-$f", "$h-moddn-$f", "$h-search-$f", "$h-unbind-$f" ]
);
...
#====================================================================
# Parse results ### Also updated for OpenDS -ADR
#====================================================================
my $entr;
foreach $entr ( $search->entries ) {
	...
	foreach $attr ( sort $entr->attributes ) {
		( my $attrname = $attr) =~ s/$h-//;
		$attrname =~ s/-$f//;
		my $attrvalue = $entr->get_value($attr);
		print "$attrname-initiated:$attrvalue $attrname-completed:$attrvalue ";
	}
}
print "\n";
